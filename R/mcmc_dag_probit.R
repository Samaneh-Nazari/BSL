#' MCMC Sampler for DAG-Probit Models
#'
#' This function implements a Markov Chain Monte Carlo (MCMC) algorithm to learn the structure
#' of a Directed Acyclic Graph (DAG) from data, assuming a probit model for the binary
#' response variable.
#' It can also compute Bayesian Model Averaged (BMA) estimates of subject-specific causal effects.
#'
#' @param y An \eqn{n,1} numeric vector of observations from the binary response variable.
#' @param X An \eqn{n,p} numeric matrix of observations for the p covariates.
#' @param TT An integer specifying the total number of MCMC iterations.
#' @param burn An integer specifying the number of burn-in iterations to be discarded.
#' @param a A numeric hyperparameter for the prior distribution. If NULL, it defaults to q.
#' @param g A numeric hyperparameter for the prior distribution. If NULL, it defaults to 1/n.
#' @param w A numeric prior probability of edge inclusion. If NULL, it defaults to 0.5.
#' @param causal A logical value. If TRUE, BMA causal effects are computed; if FALSE, they are not.
#'
#' @return A list containing the MCMC results:
#' \itemize{
#'   \item \code{A_chain}: matrix with TT columns, each representing the (vectorized) \eqn{q,q} adjacency matrix of one visited \code{DAG}
#'   \item \code{Sigma_chain}: matrix with TT columns, each representing the (vectorized) \eqn{q,q} covariance matrix of one visited \code{DAG}
#'   \item \code{Causal_hat}: \eqn{n,q} matrix with BMA estimates of subject-specific causal effects (one row for each subject, one column for each intervened node)
#' }
#'
#' @export
mcmc_dag_probit = function(y, X, TT, burn, a = NULL, g = NULL, w = NULL, causal = TRUE){
  ...
  # a,g : hyperparamters of the prior distribution
  ...
  # Set hyperparameters of the prior distribution(if not specified)
  ...
  Chol_Sigma_0 = BSL::posterior_sigma(cbind(z_0,X), A_0, g, a)
  ...
  for(t in 1:TT){
    ...
    if(type.operator == 1){
      ...
      marg_star = BSL::marg_like_j(j = nodes_star[2], dag = A_star, X = cbind(z,X), a = a, g = g, n = n)
      marg      = BSL::marg_like_j(j = nodes_star[2], dag = A, X = cbind(z,X), a = a, g = g, n = n)
      ...
      if(type.operator == 2){
        ...
        marg_star = BSL::marg_like_j(j = nodes_star[1], dag = A_star, X = cbind(z,X), a = a, g = g, n = n) +
          BSL::marg_like_j(j = nodes_star[2], dag = A_star, X = cbind(z,X), a = a, g = g, n = n)

        marg = BSL::marg_like_j(j = nodes_star[1], dag = A, X = cbind(z,X), a = a, g = g, n = n) +
          BSL::marg_like_j(j = nodes_star[2], dag = A, X = cbind(z,X), a = a, g = g, n = n)
    ...
    Chol_Sigma = BSL::posterior_sigma(Y = cbind(z,X), A, g, a)
