###############################################################################
##  sim_part1b1_q40.R — q=40, n in {100, 200}
###############################################################################
...
run_one_rep <- function(q, n, seed_offset = 0) {
  ...
  A_g <- ges_like(dat$X, max_iter = 12)
...
cat("\n=== q=40 simulation (n=100, 200) ===\n")
t0 <- Sys.time()
results_q40a <- list()
for (n in ns_main) {
  cat(sprintf("  (q=40, n=%d) ...", n))
  reps <- vector("list", N_REP)
  for (r in seq_len(N_REP)) {
    reps[[r]] <- run_one_rep(q, n, seed_offset = 1000 * q + 100 * n + r)
  }
  results_q40a[[length(results_q40a) + 1]] <- list(q = q, n = n, reps = reps)
  cat(sprintf(" done (%.1fs total)\n",
              as.numeric(difftime(Sys.time(), t0, units = "secs"))))
}
saveRDS(results_q40a, "results_q40a.rds")
cat("Saved results_q40a.rds\n")
